...
function MyPlugin:Constructor()
    ...
    -- Load the player's saved level
    local lastLevel = self:LoadPlayerLevel();
    local currentLevel = self.player:GetLevel();

    -- Display the message
    if lastLevel then
        Turbine.Shell.WriteLine("You are level " .. currentLevel .. ", you were last level " .. lastLevel);
    else
        Turbine.Shell.WriteLine("Welcome! You are level " .. currentLevel);
    end
    ...
    self.player.LevelChanged = function(sender, args)
        ...
        Turbine.Shell.WriteLine("Your new level is " .. newLevel);
    end
	
	 -- Check for level mismatch on login
    self:CheckLevelMismatch(); -- Check for level mismatch
	...
	menu:Activate(); -- Make the menu active and bring it to the front
	menu:SetZOrder(1); -- Bring the menu to the front of the z-order
	
	self.questLabel = Turbine.UI.Label();
    self.questLabel:SetParent(menu);
	...
	menu.PositionChanged = function(sender, args)
	...
	if self:IsGameOver() then
		...
		checkBox0:SetChecked(false); -- Uncheck checkBox0
	...
	function MyPlugin:CheckHP()
		...
		if currentHP <= 0 then
			...
			checkBox0:SetChecked(false); -- Uncheck checkBox0
			self:SaveCheckBoxState(false); -- Save the state
			checkBox0:SetVisible(false); -- Force UI refresh
			checkBox0:SetVisible(true); -- Force UI refresh
			self:StopTrackingQuests(); -- Stop tracking quests
		end
	end
...
-- Function to check level mismatch and show game over screen
function MyPlugin:CheckLevelMismatch()
    local lastLevel = self:LoadPlayerLevel();
    local currentLevel = self.player:GetLevel();
