    ...
    function self:OpenMenu()
        if menu and menu:IsVisible() then
            ...
            if not menu then
				...
				checkBox0:SetPosition(1000, 1000);  -- Adjusted position
...
function MyPlugin:CheckHP()
    if not self.challengeActive then
        return; -- Do nothing if the challenge isn't active
    end
...
function MyPlugin:StartMonitoringHP()
    ...
    self:CheckHP(); -- Perform an initial HP check
end
...
function MyPlugin:SaveChallengeState(isActive)
    Turbine.PluginData.Save(Turbine.DataScope.Character, "ChallengeState", isActive);
end

function MyPlugin:LoadChallengeState()
    Turbine.PluginData.Load(Turbine.DataScope.Character, "ChallengeState", function(data)
        if data then
            self.challengeActive = data;
        else
