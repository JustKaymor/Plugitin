...
function MyPlugin:Constructor()
	...
	self:LoadPluginState();

	-- Uptime tracking
	local pluginStartTime = Turbine.Engine.GetGameTime(); -- Timestamp when the plugin starts
	local totalPluginUptime = 0; -- Total tracked uptime across sessions
	...
	self:LoadData();
			...
			-- Add a label for uptime display
	local uptimeLabel = Turbine.UI.Label();
	uptimeLabel:SetParent(self.menu);
	uptimeLabel:SetSize(500, 30);
	uptimeLabel:SetPosition(100, 490); -- Position inside the menu
	uptimeLabel:SetText("Plugin Uptime: Calculating...");
	uptimeLabel:SetFont(Turbine.UI.Lotro.Font.Verdana16);
	uptimeLabel:SetTextAlignment(Turbine.UI.ContentAlignment.MiddleLeft);
	uptimeLabel:SetForeColor(Turbine.UI.Color.White);
	uptimeLabel:SetVisible(true);
	
	function MyPlugin:UpdateUptimeDisplay()
		-- Calculate session uptime
		local currentTime = Turbine.Engine.GetGameTime();
		local sessionUptime = 0;

		if isTracking and trackingStartTime then
			sessionUptime = currentTime - trackingStartTime;
		end

		-- Format uptime values
		local totalSeconds = math.floor(totalPluginUptime + sessionUptime);
		local hours = math.floor(totalSeconds / 3600);
		local minutes = math.floor((totalSeconds % 3600) / 60);
		local seconds = totalSeconds % 60;

		-- Update the uptime label
		uptimeLabel:SetText(string.format(
			"Plugin Uptime: %02d:%02d:%02d", 
			hours, 
			minutes, 
			seconds
		));
	...
	self.resetButton.Click = function(sender, args)
		...
		isTracking = true;
		trackingStartTime = Turbine.Engine.GetGameTime();
	end
	...
	self.checkBox0.CheckedChanged = function(sender, args)
		local isChecked = self.checkBox0:IsChecked();

		if isChecked then
			-- Execute logic when checkbox is checked
			Turbine.Shell.WriteLine("Survival monitoring active. Good luck!");
			...
			if not isTracking then
				isTracking = true;
				trackingStartTime = Turbine.Engine.GetGameTime();
				Turbine.Shell.WriteLine("Uptime tracking started.");
			end
		else
			-- Logic when checkbox is unchecked
			Turbine.Shell.WriteLine("Survival monitoring inactive.");
			self:StopMonitoringHP(); -- Stop monitoring HP (add this function if needed)
			self:StopTrackingQuests(); -- Disable quest tracking (assumes this function exists)

			-- Stop uptime tracking
			if isTracking then
				local currentTime = Turbine.Engine.GetGameTime();
				local sessionUptime = currentTime - (trackingStartTime or currentTime);
				totalPluginUptime = totalPluginUptime + sessionUptime;

				isTracking = false;
				trackingStartTime = nil;
				Turbine.Shell.WriteLine("Uptime tracking paused.");
			end
		end
	...
	function MyPlugin:OpenMenu()
		if self.menu:IsVisible() then
			self.menu:SetVisible(false); -- Hide menu if already visible
		else
			self.menu:SetVisible(true);  -- Show menu
			self:UpdateUptimeDisplay(); -- Ensure uptime is updated on open
		end
	end	
	
	self.menu.VisibleChanged = function(sender, args)
		if self.menu:IsVisible() then
			self:UpdateUptimeDisplay();
		end
	end
	
			local uptimeTimer = Turbine.UI.Control();
	uptimeTimer:SetWantsUpdates(false);

	uptimeTimer.Update = function(sender, args)
		if self.menu:IsVisible() then
			MyPlugin:UpdateUptimeDisplay();
		end
	end

	-- Start the timer when the menu is opened
	self.menu.VisibleChanged = function(sender, args)
		if self.menu:IsVisible() then
			uptimeTimer:SetWantsUpdates(true);
		else
			uptimeTimer:SetWantsUpdates(false);
		end
	end
...
function MyPlugin:SavePluginState()
    local saveData = {
        totalPluginUptime = totalPluginUptime,
        pluginStartTime = pluginStartTime
    };

    local saveFile = Turbine.DataScope.Character; -- Save data specific to the current character
    Turbine.PluginData.Save(saveFile, "SurvivalPluginState", saveData);
end


function MyPlugin:LoadPluginState()
    local saveFile = Turbine.DataScope.Character; -- Load data specific to the current character

    Turbine.PluginData.Load(saveFile, "SurvivalPluginState", function(data)
        if data then
            totalPluginUptime = data.totalPluginUptime or 0;
            pluginStartTime = data.pluginStartTime or Turbine.Engine.GetGameTime();
        else
            -- Initialize with default values if no data exists
            totalPluginUptime = 0;
            pluginStartTime = Turbine.Engine.GetGameTime();
        end
    end);
end
