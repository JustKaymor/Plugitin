    ...
    function self:OpenMenu()
        if menu and menu:IsVisible() then
            ...
            if not menu then
				...
				checkBox0:SetPosition(50, 80);  -- Adjusted position
...
-- Function to save the start button state
...
-- Function to load the start button state
function MyPlugin:LoadStartButtonState()
    Turbine.PluginData.Load(Turbine.DataScope.Character, "StartButtonState", function(data)
        if data ~= nil then
            self.startButton:SetVisible(data); -- Set visibility based on saved state
        else
            self.startButton:SetVisible(true); -- Default to visible if no saved state
        end
    end);
end
